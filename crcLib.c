...
uint16_t calcModbusCrc16(uint8_t *ptr, uint32_t len)
{
	uint8_t calc_crc_low,calc_crc_high; 
	uint32_t uIndex;

	calc_crc_low = calc_crc_high =0xff;
	for(; len; len--)
	{
		uIndex  = calc_crc_low ^ (*ptr);
		calc_crc_low = calc_crc_high ^ auchCRCHi[uIndex];
		calc_crc_high = auchCRCLo[uIndex];
		ptr++;
	}
	return calc_crc_high << 8 | calc_crc_low;
}
